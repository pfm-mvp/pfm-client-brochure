    ...
    const state = {
      ...
      selectedGoals: ["baseline_missing", "visitors_not_buying", "street_vs_store"],
      selectedPackage: "performance",
      opportunityType: "Premium",
      values: {
        clientName: "Company",
        preparedBy: "Anna Reilander",
        segment: "Fashion Retail",
        locations: 50,
        weeklyFootfall: 8400,
        measurement: "Basic counter",
        ...
        satShare: 18,
        satBoost: 0,
        entranceCapexStore: 1500,
        entranceMonthlyStore: 21,
        captureCapexStore: 500,
        captureMonthlyStore: 10,
        captureStores: 10,
        instoreCapexPerSqm: 35,
        instoreMonthlySensor: 125,
        instoreStores: 1,
        avgStoreSqm: 150,
        instoreSqmCoveragePerSensor: 75
      }
    };
    ...
    const retailGoals = {
      ...
      groups_distort_conversion: {
        ...
        hiddenKpis: ["individual_footfall", "buying_units", "group_size", "adult_child_split", "conversion_rate", "conversion_per_buying_unit"],
        hiddenSubscriptions: ["sub_buying_units", "sub_demographic_context", "sub_sales_conversion"],
    ...
    function routeLabel() {
      ...
      return packages[state.selectedPackage]?.name || "Performance";
    }
    ...
    function solutionComponents() {
      ...
      const captureStores = captureActive ? clamp(Number(state.values.captureStores || Math.min(10, stores)), 1, stores || 1) : 0;
      ...
      const components = [
        {
          ...
          quote_note: "Premium entrance counter model pricing."
        },
        {
        ...
        },
        {
          ...
          quote_note: "Indicative in-store analytics model. Sensor count based on m2 and 75m2 coverage at 2.95m mounting height."
        }
      ];
      return components;
    }
    ...
    function renderSummary() {
      ...
      summary.innerHTML = `
        ...
        <p class="muted">${state.customerType} · ${operatingUnit()}</p>
        ...
        `}
        <div class="summary-row"><span>Odoo-ready</span><strong>Yes</strong></div>
      `;
    ...
    function renderStep2() {
      ...
      target.innerHTML = `
        <div class="content-card">
          ...
          <p class="muted">${isShopping ? "Choose the asset, leasing, flow, parking, catchment and portfolio challenges your customer recognises. PFM translates them into the right mix of sensors, smart data and asset intelligence." : "Select the business situations your customer recognises. Each card is based on real retail conversations and maps silently to KPI, insight and subscription logic."}</p>
          <div class="goal-grid">
            ${Object.entries(catalog).map(([id, g]) => `
              <div class="pain-card ${state.selectedGoals.includes(id) ? "active" : ""}" onclick="toggleGoal('${id}')">
                <div>
                  ...
                  <p class="muted" style="font-size:13px;margin-top:10px;"><strong>Sales proof:</strong> ${g.proof}</p>
                </div>
    ...
    function renderStep3() {
      ...
      target.innerHTML = `
        <div class="content-card">
          ...
          <p class="muted">${isShopping ? "This is the asset intelligence layer: sensors where accuracy matters, geo-app data where catchment matters, ANPR where mobility matters, and BI where portfolio control matters." : "This is what the customer should understand: not which metric they buy, but what PFM helps them see and improve."}</p>
          ...
          <div class="quote-box">
            ...
            <p>${isShopping ? "Use this page to separate customer-facing value from technical delivery. The customer recognises asset pains; internally we map this to sensors, smart data, ANPR, Re-ID feasibility, BI modules and n8n/Odoo output." : "Use this page as the bridge from commercial pain to PFM’s intelligence layer. The technical mapping stays available for n8n/Odoo, not for the customer conversation."}</p>
          </div>
              ...
              <details class="advanced-impact">
                ...
                <div class="advanced-body">
                  <div class="pricing-grid-v13">
                    ...
                    ${capture ? numberField("captureStores", "Capture stores in scope", Math.min(state.values.captureStores, state.values.locations)) : ""}
    ...
    function renderShoppingRoute() {
      ...
      target.innerHTML = `
        <div class="content-card">
          ...
          <p class="muted">These package names and pricing are placeholders for tomorrow's discussion. The selected performance leaks already determine which modules are relevant.</p>
          ...
          <div class="notice" style="margin-top:24px;">Route explains value level only. Budget/Premium and exact Odoo quote lines remain sales-only on submit.</div>
          <div class="page-actions"><button class="ghost-btn" onclick="go(4)">Back</button><button class="primary-btn" onclick="go(6)">Prepare submit</button></div>
        </div>
        ${renderSummary()}`;
    }
    ...
    function renderStep5() {
      ...
      target.innerHTML = `
        <div class="content-card">
          <div class="kicker">Recommended route</div>
          <h2>Choose the <span class="highlight">commercial route</span></h2>
          <p class="muted">This is still customer-facing value language. Budget/Premium template selection happens later at Sales submit.</p>
          <div class="package-grid">
            ${Object.entries(packages).map(([id,p]) => `
              <div class="package-card ${state.selectedPackage === id ? "active" : ""}" onclick="selectPackage('${id}')">
                <h3>${p.name}</h3>
                <p class="muted">${p.headline}</p>
                <div class="package-price">${euro(p.monthly * Number(state.values.locations))} / mo.</div>
                <p class="muted">${p.promise}</p>
                <div>${p.contains.map(c => `<span class="tag">${c}</span>`).join("")}</div>
              </div>
            `).join("")}
          </div>
          <div class="notice" style="margin-top:24px;">Package route explains the value level. Odoo template selection remains a sales-only submit choice on the next step.</div>
          <div class="page-actions"><button class="ghost-btn" onclick="go(4)">Back</button><button class="primary-btn" onclick="go(6)">Prepare submit</button></div>
        </div>
        ${renderSummary()}`;
    }
    ...
    function renderStep6() {
      ...
      target.innerHTML = `
        <div class="content-card">
          <div class="kicker">Sales submit</div>
          <h2>Ready for <span class="highlight">n8n / Odoo</span></h2>
          <p class="muted">The buying journey is customer-facing. This final block is sales-only and determines the right Odoo output.</p>
          <div class="submit-grid">
            <div>
              <div class="form-grid">
                <div>
                  <label>Opportunity</label>
                  <select id="opportunityType"><option>Premium</option><option>Budget</option></select>
                </div>
                <div>
                  <label>Operating unit</label>
                  <input value="${operatingUnit()} · ID ${OPERATING_UNIT_MAP[operatingUnit()]}" disabled>
                </div>
                <div>
                  <label>Salesperson</label>
                  <input value="${state.values.preparedBy} · ID ${salespersonId()}" disabled>
                </div>
                <div>
                  <label>Odoo template</label>
                  <input value="${p.pfm_internal.odoo_template}" disabled>
                </div>
              </div>
              <div class="notice" style="margin-top:20px;">Human review stays mandatory. This payload should create or enrich a lead/proposal, not send a final quote without sales validation.</div>
              <div class="page-actions"><button class="ghost-btn" onclick="go(5)">Back</button><button class="primary-btn" onclick="downloadPayload()">Download payload</button></div>
            </div>
            <div class="payload-box" id="payloadBox">${JSON.stringify(p, null, 2)}</div>
          </div>
        </div>
        ${renderSummary()}`;
      const opp = document.getElementById("opportunityType");
      if (opp) {
        opp.value = state.opportunityType;
        opp.addEventListener("change", () => { state.opportunityType = opp.value; renderStep6(); });
      }
    }
    ...
    document.querySelectorAll(".customer-card").forEach(card => card.addEventListener("click", () => {
      ...
      if (state.customerType === "Retail Chain") state.selectedGoals = ["baseline_missing", "visitors_not_buying", "street_vs_store"];
